...
import { type StateBorders, getData, isPointInPoly, whichState } from "./util";
...
const mockStatesData: Array<StateBorders> = [
	...
	}, // Shares an edge with MockState1
];
...
test('whichState returns "no state found" for a point outside all states', () => {
	...
	expect(whichState(pointOutsideAnyState, mockStatesData)).toBe(
		"no state found",
	);
});
...
test("whichState handles points on the edge of a state correctly", () => {
	...
	expect(whichState(pointOnEdgeOfMockState1, mockStatesData)).toBe(
		"MockState1",
	);
});
